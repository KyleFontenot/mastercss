      ...
      for (const eachVariableName of syntaxRule.variableNames) {
        ...
        if (this.css.themeLayer.rules.find(({ name }) => name === eachVariableName)) {
          ...
          const addNative = (mode: string, _variable: TypeVariable) => {
            ...
            let prefixCssRuleText: string
            let endCurlyBracketCount = 1
            if (mode) {
              switch (this.css.config.modes?.[mode]) {
                case 'media':
                  prefixCssRuleText = `@media(prefers-color-scheme:${mode}){body`
                  endCurlyBracketCount++
                  break
                case 'host':
                  prefixCssRuleText = `:host(.${mode}) body`
                  if (!variable.value && this.css.config.defaultMode === mode) {
                    prefixCssRuleText += ',:host body'
                    isDefaultMode = true
                  }
                  break
                case 'class':
                  prefixCssRuleText = `.${mode} body`
                  if (!variable.value && this.css.config.defaultMode === mode) {
                    prefixCssRuleText += ',body'
                    isDefaultMode = true
                  }
                  break
                default:
            ...
            const cssRuleText = `${prefixCssRuleText}{--${eachVariableName}:${String(_variable.value)}${'}'.repeat(endCurlyBracketCount)}`
